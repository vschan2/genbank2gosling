...
4. **GC skew** (`stage4_gc_skew.py`) — sliding-window `(G−C)/(G+C)` plus cumulative skew, plus windowed GC content `(G+C)/window`. Window/step are **fixed per cluster**, read from `scripts/gc_skew_config.json` (bacteria: 1000 bp window, non-overlapping; mitogenome: 150 bp window, 30 bp step) rather than hardcoded, because mitogenomes and bacterial chromosomes differ by ~2 orders of magnitude in length. A cluster absent from the config falls back to the config's `default` auto-scale formula (`window ≈ length/1000`, `step ≈ window/5`), so a newly added cluster doesn't require code changes. Since every genome is circular, windows running past the sequence end wrap onto the start rather than truncating (reported `end` stays capped at genome length; only the G/C counts wrap). See `specs/spec-003-gc-window-metrics.md`. Write `<genome>_gc_skew.bedgraph`.
