...
| GC skew | `<genome>_gc_skew.bedgraph` | three separate slim tracks: two `line` marks (windowed, split into positive/negative halves) + one `area` mark (cumulative) | Columns are `chrom, start, end, gc_skew, cumulative_gc_skew`. `cumulative_gc_skew` is an unbounded running sum (observed range roughly ±600 to ±50,000 across sampled genomes) while `gc_skew` stays within ±1, so each series gets its own single-scale track rather than one dual-axis overlay. The windowed track is split into two `line` tracks — `domain: [0, 1]` (positive) and `domain: [0, -1]` + `flip: true` (negative) — each with its own color (`GC_SKEW_POSITIVE_COLOR`/`GC_SKEW_NEGATIVE_COLOR`, `components/trackStyles.ts`), stacked so their zero-lines meet at a shared boundary and read as one continuous diverging skew line. |
