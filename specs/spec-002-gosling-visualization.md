...
| GC skew | `<genome>_gc_skew.bedgraph` | line mark + area mark, two separate slim tracks | Columns are `chrom, start, end, gc_skew, cumulative_gc_skew`. **Revised during mitogenome page implementation**: originally planned as one track with both series overlaid, but `cumulative_gc_skew` is an unbounded running sum (observed range roughly ±600 to ±50,000 across sampled genomes) while `gc_skew` stays within ±1 — overlaying them on one y-scale is a dual-axis chart and flattens the windowed line to invisible. Implemented as two adjacent single-scale tracks instead (thin line for windowed, filled area for cumulative), each 40px tall — the row-group height cost is minor and correctness won out over compactness. |
