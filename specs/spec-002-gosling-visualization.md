...
| GC skew | `<genome>_gc_skew.bedgraph` | four separate slim tracks: two `line` marks (windowed skew, split into positive/negative halves) + one `line` mark (windowed GC content) + one `area` mark (cumulative skew) | Columns are `chrom, start, end, gc_skew, cumulative_gc_skew, gc_content`. `cumulative_gc_skew` is an unbounded running sum (observed range roughly ±600 to ±50,000 across sampled genomes) while `gc_skew` and `gc_content` stay within [-1, 1] and [0, 1] respectively, so each series gets its own single-scale track rather than one dual-axis overlay. The windowed skew track is split into two `line` tracks — `domain: [0, 1]` (positive) and `domain: [0, -1]` + `flip: true` (negative) — each with its own color (`GC_SKEW_POSITIVE_COLOR`/`GC_SKEW_NEGATIVE_COLOR`, `components/trackStyles.ts`), stacked so their zero-lines meet at a shared boundary and read as one continuous diverging skew line. The GC content track (`GC_CONTENT_COLOR`) is stacked directly below that pair, sharing its `[0, 1]` domain/height, before the cumulative-skew area track. |
| Sequence | mitogenome: `<genome>_sequence.json` (positioned records, see prerequisite change above); bacteria: multivec tileset served by higlass-server | overlay of a `bar` mark + a `text` mark, both marks gated by the same zoom-level/cell-width visibility rule; example: https://gosling.js.org/?example=SEQUENCE | Bacterial tileset UID = sanitized manifest `id` (`.` → `-`), matching `register-tilesets.sh`. No `row` visual channel — all 4 bases' bars stack in one lane, which gives semantic zoom for free from the tile pyramid itself: zoomed out, clodius-pooled per-base counts are fractional, so the stacked bars read as a composition/proportion chart; zoomed to ~1bp/pixel the one-hot data collapses the stack to a single full-height color block per base, with the letter overlaid by the `text` mark. |
...
- **Track `height` is the total rendered height, divided among a nominal `row` channel's categories** — corrected during the full bacterial page build (step 6) after the initial mitogenome-era assumption (`height × <row count>` = total) turned out backwards and rendered the bacterial multivec track as a near-invisible sliver. Confirmed by reading `drawBar`'s `rowHeight = trackHeight / rowCategories.length` in `gosling.js`'s compiled source (`node_modules/gosling.js/dist/compile-*.js`): `trackHeight` there is the spec's `height` value directly, not a multiplied allocation. So a 4-category multivec track needs `height` set to the *desired total*, not a per-row amount — e.g. `TRACK_HEIGHTS.multivec` in `trackStyles.ts` is 60px total (~15px per A/T/G/C row), not 60px per row. `BacterialSmokeTest.tsx`'s `rowHeight` variable name is now known to be misleading (it computes 1/4 of the intended total, not a true per-row height) but was left as-is since it's smoke-test-only code that still renders acceptably by coincidence (full-viewport-height budget). Because Gosling renders to a fixed-pixel canvas rather than a CSS-responsive element, fitting a view to the actual browser viewport means computing `height`/`width` from the viewport in an effect with a resize listener, not passing a static number — see `useViewportWidth.ts` for the pattern used across pages.
...
- ~~GC content column exists in the data but has no track yet~~ — **resolved**: `gcSkewTracks()` in `web/lib/buildSpec.ts` now appends `'gc_content'` to `headerNames` and adds a fourth `line` track (`GC_CONTENT_COLOR`, reusing `TRACK_HEIGHTS.gcSkewWindowed`), stacked between the windowed skew pair and the cumulative-skew area track since it shares the `[0, 1]` domain with the positive-skew half. Applies to both cluster pages (shared function). `tsc --noEmit` clean; not yet visually confirmed in-browser.
...
5. ~~**Mitogenome JSON page**~~ — **done**. Full `mitogenome/page.tsx` with `GenomePicker` + `ClusterView` + `buildSpec.ts`, reading `output/mitogenome_cluster/` via `/api/genomes/[cluster]/[...path]/route.ts` (server-side file reads, path-traversal guarded). Live re-render on checkbox toggle (no "Generate" button needed — mitogenome files are small enough that re-fetching per toggle isn't janky). GC skew implemented as two separate tracks rather than one overlaid track — see the revised "Per-genome track composition" table above. Verified: `tsc`/`next build` clean, all three API routes curl-checked, page confirmed rendering correctly in-browser by the user. Not yet built: PNG/PDF export (step 7).
6. ~~**Next up — complete/flexible bacterial page**~~ — **done**: extend the smoke test into the real `bacterial/page.tsx` — full picker over all 7 genomes (not just the one hardcoded one), `register-tilesets.sh` run over the full set (not ad hoc single-genome commands), reusing the row-group patterns from step 5. Worth resolving the high-zoom blanking bug before or during this step, since it'll affect every genome, not just the smoke-test one.
7. ~~**GC content track**~~ — **done**: extended `gcSkewTracks()`'s `headerNames`, added the fourth track and the `GC_CONTENT_COLOR` constant. `tsc --noEmit` clean; not yet visually confirmed in-browser.
8. **(Stretch, optional)** PNG/PDF download button, live-vs-button picker UX decision, `/api/run-pipeline` route for triggering stage scripts from the UI — none of these block the immediate goal, so they can slip past it if time-constrained.
